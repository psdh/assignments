...
select(X,P,No,Lis,L1) :- (No>0, [Head|Tail] = Lis, element_at(He, Head,2),isNotThere(L1,He),[A,B,_,D,E]=Head ,E>= X ,D=<P) -> (append(L1,[He],L2), No1 is No -1 ,write(A),nl, select(X,P,No1,Tail,L2))
;
([Head|Tail] = Lis, select(X,P,No,Tail,L1)).

assign(X,L,Q,L1) :- element_at(No,X,1), element_at(E,X,2),element_at(Pa,X,3),list(Gh), select(E,Pa,No,Gh,L1),Q1 is Q -1, assign_projects(L,Q1,L1) .

assign_projects(List1,0,L1).

assign_projects(List1,Q) :- assign_projects(List1,Q,L1).
assign_projects(List1,Q,L1) :-   Q>0, [X|L] = List1,write('Project :'),nl, (assign(X,L,Q,L1) ; mia).      
